...
fn application_body(
    ...
    let auth = match action.auth {
        ...
        AuthKind::Ownership => [
            ...
            &marker("application-auth-decoded"),
            "if signed.act_as.is_some() { return Err(StateAccessError::Rejected(jamscript_runtime_core::RuntimeError::ActAsUnsupported.code())); };",
            "let nonce_key = jamscript_runtime_core::ownership_nonce_key(&signed.controller).map_err(|_| StateAccessError::Backend)?;",
            "let nonce_bytes = context.state().get(&nonce_key)?.unwrap_or_default();",
            "let expected_nonce = match nonce_bytes.as_slice() { [] => 0u64, bytes if bytes.len() == 8 => u64::from_le_bytes(bytes.try_into().map_err(|_| StateAccessError::Backend)?), _ => return Err(StateAccessError::Backend) };",
            "let verified = jamscript_runtime_core::verify_signed_action_v2(signed, context.network_domain(), SERVICE_KEY, ACTION_SELECTOR, Some(expected_nonce)).map_err(|_| StateAccessError::Rejected(jamscript_runtime_core::RuntimeError::InvalidOwnershipAuthorization.code()))?;",
            "context.set_ownership(verified.owner.clone(), verified.controller.clone());",
            "context.constrain_valid_until(verified.valid_until);",
            "let next_nonce = expected_nonce.checked_add(1).ok_or(StateAccessError::Backend)?;",
            "context.state().set(&nonce_key, &next_nonce.to_le_bytes())?;",
