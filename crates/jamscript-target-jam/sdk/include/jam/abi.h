...
#ifndef JAMSCRIPT_JAM_ABI_H
#define JAMSCRIPT_JAM_ABI_H
...
/* Export names are kept stable because they are part of existing artifacts. */
#define JAM_REFINE __attribute__((used)) minijam_refine_output minijam_refine(void)
#define JAM_ACCUMULATE __attribute__((used)) void minijam_accumulate(void)
...
minijam_status minijam_result(size_t index, void *output, size_t output_capacity,
                              size_t *output_size);
minijam_status minijam_storage_read(const void *key, size_t key_size,
                                    void *output, size_t output_capacity,
                                    size_t *output_size);
/*
 * Accumulation-only cross-Service read. Refine must carry and verify an
 * authenticated dependency witness instead of reading another Service.
 */
