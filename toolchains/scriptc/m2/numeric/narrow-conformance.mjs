...
const output = resolve(root, "out-narrow-conformance");
...
const result = await compileLibrary({
  profilePath: resolve(root, "narrow-conformance.profile.json"),
  outDir: output,
  outPath: resolve(output, "numeric-narrow-conformance.a"),
  emitIr: true,
});
if (!result.ok) throw new Error(JSON.stringify(result.diagnostics, null, 2));
const binary = resolve(output, "numeric-narrow-conformance");
const cc = process.env.CC || "clang";
const compile = spawnSync(cc, ["-no-pie", resolve(root, "narrow-conformance.c"), result.archivePath, "-lm", "-pthread", "-l:libbsd.so.0", "-o", binary], { encoding: "utf8" });
